...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $query = "SELECT * FROM bookmarks WHERE id_post = '$id_post' AND id_users = '$id_users'";
    $hasil = mysqli_query($conn, $query);

    if (mysqli_num_rows($hasil) == 0) {
        // User belum memberikan like, tambahkan like ke database
        $query = "INSERT INTO bookmarks (id_post, id_users) VALUES ('$id_post', '$id_users')";
        $hasil = mysqli_query($conn, $query);

        if ($hasil) {
            // Like berhasil ditambahkan
            // Update jumlah like pada tabel post
            $query = "UPDATE post SET bookmarks = bookmarks + 1 WHERE id_post = '$id_post'";
            $hasil = mysqli_query($conn, $query);
        ...
        echo "Anda sudah menambahkan pada bookmarks ";
    }
}
